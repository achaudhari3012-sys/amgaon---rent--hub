...
import { CLOTHING_ITEMS } from '../types';
        ...
        {CLOTHING_ITEMS.map((item) => (
          <div key={item.id} className="glass-card rounded-xl overflow-hidden shadow-lg p-4">
            ...
            <div className="flex justify-between items-center mt-4">
              <span className="bg-white/10 px-3 py-1 rounded-full text-sm border border-white/20">{item.category}</span>
              <span className="text-yellow-500 font-medium">{item.priceRange}</span>
            </div>
          </div>
        ))}
      </div>
